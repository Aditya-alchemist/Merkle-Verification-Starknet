 ...
 * ```
 */
export const hashPair = (a, b, hashType = HashType.Pedersen) => BigInt(a) - BigInt(b) >= 0n
 ...
 * ```
 */
export function hashDataToHex(data, hashType = HashType.Pedersen) {
    let aa = [];
 ...
 * ```
 */
export function hashDataToBigint(data, hashType = HashType.Pedersen) {
    let aa = [];
    if (Array.isArray(data)) {
        aa = data;
    }
    ...
    return hashType == HashType.Pedersen ? BigInt(ec.starkCurve.computeHashOnElements(aa)) : computePoseidonHashOnElements(aa);
}
